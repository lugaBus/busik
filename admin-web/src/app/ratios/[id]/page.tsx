...
import { ratiosApi, Ratio, CreateRatioDto } from '@/api/ratios';
import { authService } from '@/auth/auth.service';
import { createI18nText, I18nText, Language } from '@/utils/i18n';

export default function RatioFormPage() {
  ...
  const [formData, setFormData] = useState<CreateRatioDto>({
    name: createI18nText(),
    slug: '',
  ...
  const loadRatio = async () => {
    try {
      setLoading(true);
      const ratio = await ratiosApi.getById(id);
      setFormData({
        name: ratio.name || createI18nText(),
        slug: ratio.slug,
        description: ratio.description || createI18nText(),
      });
    } catch (error) {
      console.error('Failed to load ratio:', error);
      alert('Failed to load ratio');
      router.push('/ratios');
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await ratiosApi.update(id, formData);
      } else {
      ...
      alert(error.response?.data?.message || 'Failed to save ratio');
    } finally {
      setSaving(false);
    }
  };
