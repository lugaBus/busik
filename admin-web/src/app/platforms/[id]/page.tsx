...
import { platformsApi, Platform, CreatePlatformDto } from '@/api/platforms';
...
export default function PlatformFormPage() {
  ...
  const loadPlatform = async () => {
    try {
      setLoading(true);
      const platform = await platformsApi.getById(id);
      setFormData({
        name: platform.name || createI18nText(),
        slug: platform.slug,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await platformsApi.update(id, formData);
      } else {
