...
import { authService, RegisterCredentials } from '@/auth/auth.service';
...
export default function RegisterPage() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!confirmPassword) {
      errors.confirmPassword = language === 'en' ? 'Please confirm your password' : language === 'ua' ? 'Підтвердіть пароль' : 'Подтвердите пароль';
    } else if (password !== confirmPassword) {
      errors.confirmPassword = language === 'en' ? 'Passwords do not match' : language === 'ua' ? 'Паролі не співпадають' : 'Пароли не совпадают';
    }
        ...
        <div className="auth-container">
          <div className="auth-card">
            <h2 className="auth-title">
              {language === 'en' ? 'Register' : language === 'ua' ? 'Реєстрація' : 'Регистрация'}
            </h2>
            <form 
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder={language === 'en' ? 'At least 6 characters' : language === 'ua' ? 'Мінімум 6 символів' : 'Минимум 6 символов'}
                />
              ...
              <div className="form-group">
                <label className="form-label">
                  {language === 'en' ? 'Confirm Password' : language === 'ua' ? 'Підтвердити пароль' : 'Подтвердить пароль'}
                </label>
                <input
                  type="password"
                  ...
                  placeholder={language === 'en' ? 'Confirm your password' : language === 'ua' ? 'Підтвердіть пароль' : 'Подтвердите пароль'}
                />
              ...
              <div className="form-group">
                <label className="form-label">
                  {language === 'en' ? 'First Name' : language === 'ua' ? 'Ім\'я' : 'Имя'} ({language === 'en' ? 'Optional' : language === 'ua' ? 'Необов\'язково' : 'Необязательно'})
                </label>
                <input
                  type="text"
                  value={firstName}
                  ...
                  placeholder={language === 'en' ? 'John' : language === 'ua' ? 'Іван' : 'Иван'}
                />
              </div>
              <div className="form-group">
                <label className="form-label">
                  {language === 'en' ? 'Last Name' : language === 'ua' ? 'Прізвище' : 'Фамилия'} ({language === 'en' ? 'Optional' : language === 'ua' ? 'Необов\'язково' : 'Необязательно'})
                </label>
                <input
                  type="text"
                  value={lastName}
                  ...
                  className="form-input"
                  placeholder={language === 'en' ? 'Doe' : language === 'ua' ? 'Петренко' : 'Петренко'}
                />
              </div>
              <button
                type="button"
              ...
              >
                {loading
                  ? (language === 'en' ? 'Registering...' : language === 'ua' ? 'Реєстрація...' : 'Регистрация...')
            ...
            <div className="auth-footer">
              <span className="auth-footer-text">
                {language === 'en' ? 'Already have an account?' : language === 'ua' ? 'Вже є обліковий запис?' : 'Уже есть аккаунт?'}
              </span>
